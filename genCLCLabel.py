...
def run_clc(config):
  ...
  output_txt = config["paths"]["output_file"]

  process(image_folder, output_txt)

def process(image_folder, output_txt):
  with open(output_txt, 'a', encoding='utf-8') as outfile:
          ...
          if ocr_response.status_code == 200:
              ...
              if ocr_data.get("is_success") and ocr_data["data"].get("result_ocr_text"):
                  ...
                  if is_width_larger_than_height(image_path) and combined_text != "No result OCR":
                      print("reverse this one")
                      print(f"text before reverse: {combined_text}")
                      combined_text = combined_text[::-1]  # Reverse the text
                  print(combined_text)
                  # Write the updated line to the output file
                  outfile.write(f"{image}\t{combined_text}\n")
              else:
  ...
  print("Processing complete. Updated file written to:", output_txt)